...
int analyzer( const char* runFile ){
  ...
  int sample_size = 256;  //size of the fake waveform from the WIB
  double triggerRate = 1; // in Hz
  double frameLength = 2560.; // in 2 MHz samples   //guanqun: number of ticks in 1 frame.
  double framesize = frameLength*0.5e-6;  //the length of 1 frame. The length of 1 tick is 0.5us.
  ...
  double totEvent = ceil(entries/NFEMs); //calculate the total number events based on entries
  int EventRange = ceil(totEvent/EventBin);  //calcualte num of binning for the "event" axis assuming using bin size 'EventBin'.
 ...
 // gROOT->cd();  //make sure all newly created histos are associated with gROOT directory.
  ...
  //internal comparison of trigger sample within 1 event
   TH2D* hTriggerSamComp = new TH2D("hTriggerSamComp","Trigger sample difference between first slot and other slots within 1 event;Slot;trigger sample difference; entries", NFEMs-1, firstFEM+1, firstFEM+NFEMs,100,0,1000);


  TH2D *hFrameTrigger=new TH2D("hFrameTrigger", "TriggerRate calculated by the frame difference", NFEMs, firstFEM, firstFEM+NFEMs, (int)2*triggerRate, 0, 2*triggerRate);
 ...
 //hFrames->SetDirectory(gROOT);  
  ...
  // how those variables changes with time for each FEM or channel.
   ...
   std::cout << "histograms are generated" <<std::endl; 
 ...
 //open the reference root file.
  ...
  //grab the checksum for sample_sized ADC words in channels.
  std::vector<long int>* pointer_checksum_channel;
  refFile->GetObject("checksum_channel", pointer_checksum_channel);
  std::vector<long int> ref_checksum_channel = *pointer_checksum_channel;
...
//    if(event > MAX_EVENT) MAX_EVENT = event;
    ...
    if(hwaveform->size() != 64) std::cout << "FEM " << slot << " only has waveforms for " << hwaveform->size() << " channels." <<std::endl; 
    //get the Max, Min, RMS of the waveform
    //loop over all the channels
    for(int k=0; k< (int)hwaveform->size(); k++){ 
	int samplenum = 0;
        double Max, Min, Mean, RMS;
	int channelnumber = (slot - firstFEM)*64 +k;
	...
	if(samplenum == 0) std::cout << "Channel " << channelnumber << " has no waveform, SKIP IT!!" << std::endl;
		...
		if(is_reference && i==0){
			...
			//calculate the checksum of 'sample_size' ADC words for each word.
    ...
    if( prev_frame[(i-1)%NFEMs] != frame ) totalTriggers++;

    int deltaEvent = event - prevEvent;
    hDeltaEvent->Fill(slot,deltaEvent);
    hTDeltaEvent->Fill(event,slot, deltaEvent);
    if(deltaEvent == 0){ //if it's in the same event
	if((i-1)%NFEMs == (NFEMs-1)) std::cout<< "New event doesn't start from first FEM slot" << std::endl;
	else{
		hFrameComp[(i-1)%NFEMs]->Fill(prev_frame[0] - frame);
		hTriggerSamComp->Fill(slot, triggersample - TSample_firstSlot);
	}
    }else TSample_firstSlot=triggersample;



    //use Frame difference between this event and previous event to calculate the trigger rate.
...
//***************REFERENCE RUN: write histograms into the output file******************************************.
  //should be commented if it's not reference running.
/*  std::cout << "This is reference run!!" << std::endl;
  ...
  gPad->SetLogz(1);  
  gStyle->SetOptStat(0);
  gStyle->SetNumberContours(99);  //set the number of colors
